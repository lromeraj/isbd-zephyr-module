...
static inline bool _evt_parse_areg( const char *buf, isbd_dte_evt_t *evt );
static inline bool _evt_parse_ciev( const char *buf, isbd_dte_evt_t *evt );
static inline bool _evt_parse_ring( const char *buf, isbd_dte_evt_t *evt, bool verbose );

isu_dte_err_t isbd_dte_evt_wait( isu_dte_t *dte, isbd_dte_evt_t *event, uint32_t timeout_ms ) {

  char buf[ 32 ];
  event->id = ISBD_DTE_EVT_UNK;
...
static inline bool _evt_parse_ciev( const char *buf, isbd_dte_evt_t *evt ) {
  ...
  if ( ret == 2 ) {
    ...
    if ( ciev_evt == 0 ) {
      evt->id = ISBD_DTE_EVT_SIGQ;
      evt->sigq = ciev_val;
    } else if ( ciev_evt == 1 ) {
      evt->id = ISBD_DTE_EVT_SVCA;
      evt->svca = ciev_val;
    }
  
  }

  return evt->id != ISBD_DTE_EVT_UNK;
}
